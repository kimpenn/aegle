        ...
        for j in range(cell_coord_num):
            ...
            if cell_size_current != 0:
                single_cell_intensity = (
                    np.sum(channel[tuple(cell_coord[j])]) / cell_size_current
                )
                single_cell_intensity_z = (
                    np.sum(channel_z[tuple(cell_coord[j])]) / cell_size_current
                )
...
def get_schema_url(ome_xml_root_node: ET.Element) -> str:
    if m := schema_url_pattern.match(ome_xml_root_node.tag):
        return m.group(1)
    raise ValueError(
        f"Couldn't extract schema URL from tag name {ome_xml_root_node.tag}"
    )
...
def get_physical_dimension_func(
    ...
    def physical_dimension_func(
        img: aicsimageio.AICSImage,
    ) -> Tuple[UnitRegistry, Quantity]:
        """
...
def single_method_eval(
    img: Image, mask_dict: dict, output_dir: Path
) -> Tuple[Dict[str, Any], float, float]:
    logging.info("Calculating single-method metrics v1.5 revised by DK for PennTMC")
    ...
    fg_bg_image = Image.fromarray(img_binary.astype(np.uint8) * 255, mode="L").convert(
        "1"
    )
    ...
    for channel in range(metric_mask.shape[0]):
        ...
        if channel_names[channel] == "Matched Cell":
            ...
            try:
                matched_fraction = mask_xmldict["OME"]["StructuredAnnotations"][
                    "XMLAnnotation"
                ]["Value"]["OriginalMetadata"]["Value"]
            except:
    ...
    file_name = (
        "/workspaces/codex-analysis/0-phenocycler-penntmc-pipeline/aegle/pca.pickle"
    )
