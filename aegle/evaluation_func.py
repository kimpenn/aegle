...
def fraction(img_bi, mask_bi):
    ...
    if len(img_bi.shape) == 3:
        background_all = (
            img_bi.shape[0] * img_bi.shape[1] * img_bi.shape[2] - foreground_all
        )
...
#     repaired_num = 0
#     for cell_label, cell_coords in cell_coords_dict.items():
...
#                             current_cell_coords,
#                             cell_membrane_coords_dict.get(cell_label, np.array([])).T,
#                             nucleus_coords,
#                             mismatch_repair=1
#                         )
...
#     # Proceed to construct matched masks from the matched lists...
#     # matched_cell_mask = ...
...
def get_matched_masks(cell_mask, nucleus_mask):
    ...
    if repaired_num > 0:
        logging.debug(
            str(repaired_num) + " cells repaired out of " + str(len(cell_coords))
        )
